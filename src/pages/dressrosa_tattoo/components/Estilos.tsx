    ...
    <section id="estilos" style={{ padding: '108px 52px', background: theme.bg }}>
      <div style={{ maxWidth: '1240px', margin: '0 auto' }}>
        <div data-reveal style={{ textAlign: 'center', marginBottom: '64px' }}>
          ...
          <h2 style={{ fontFamily: fonts.title, fontSize: 'clamp(40px, 5vw, 68px)' }}>Estilos que Tatuo</h2>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1px', background: 'rgba(201,169,110,0.15)' }} data-reveal>
          {items.map((estilo: any, i: number) => (
            ...
            >
              <div style={{ fontFamily: fonts.title, fontStyle: 'italic', fontSize: '28px', color: theme.accent, marginBottom: '20px' }}>{estilo.num}</div>
              <h3 style={{ fontFamily: fonts.title, fontSize: '26px', marginBottom: '18px', color: theme.text }}>{estilo.name}</h3>
